...
### `live-recovery` 的撤回与再抬（2026-09-26 02:45）

- 本窗口第一版把 `live-recovery` 900→1200，两路独立审查同步撤回：理由是它当时**没有属于自己的
  失败记录**（五次干净实测 477–598 s，占 900 s 档 53–66%），而 R2 要求按该套件自己的实测与 CI
  墙钟定档、倍率不是配额。撤回是对的。
- 随后 run
  [#150](https://github.com/MaginaLW/ai-agent-dotfiles/actions/runs/36164636463) 在分片 3 上给出
  属于它的杀点：注解 `discovered=27 passed=26 failed=0 timed-out=1
  failing=[live-recovery.tests.ps1:timed-out:exit=-1]`，而同一晚的 #148（该套件 1200 s 档）与
  #149（900 s 档）都通过——**它确实在 900 s 档上会间歇性地被杀**（它自己的实现倍率超过约 1.51）。
- 处置：按同一「约为干净最大值两倍」的规则抬到 **1200 s**；并按审查给出的条件把 shard 3 的
  `timeout-minutes` 230→**235**，把这一档吃掉的 300 s 从合同之上的余量里还回来
  （12495 + 1605 = 14100 s，余量与撤回前一致）。分片 3 合同 12195→**12495 s**，聚合
  30255→**30555 s**。
