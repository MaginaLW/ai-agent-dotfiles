...
try {
    ...
    Assert-TestCondition ($agentsText.Contains('For maintenance validation, run schema 3 sync DryRun only inside the internal sandbox') -and $agentsText.Contains('expected rejection is not isolation')) 'AGENTS.md requires sandboxed validation without assuming released public DryRun rejects'
