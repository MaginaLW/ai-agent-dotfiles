...
try {
    ...
    Assert-TestCondition ($agentsText.Contains('returns `safety-protocol-upgrade-required` before backup or mutation')) 'AGENTS.md documents the production interlock token before backup or mutation'
    Assert-TestCondition ($claudeText.Contains('`safety-protocol-upgrade-required` before backup or mutation')) 'CLAUDE.md documents the production interlock token before backup or mutation'
    Assert-TestCondition ($readmeText.Contains('they return `safety-protocol-upgrade-required`')) 'README.md documents that Apply, rollback, retirement, and standalone backup stay interlocked'
    Assert-TestCondition ($readmeText.Contains('Generate and review an external plan. Apply remains interlocked in Phase 0.')) 'README.md documents the external reviewed-plan DryRun flow'
