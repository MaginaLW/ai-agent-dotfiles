...
try {
    ...
    Assert-TestCondition ($agentsText.Contains('returns `safety-protocol-upgrade-required` before traversal or')) 'AGENTS.md documents the production interlock token before traversal or mutation'
    ...
    Assert-TestCondition ($readmeText.Contains('Generate and review an external plan. Apply remains interlocked.')) 'README.md documents the external reviewed-plan DryRun flow'
