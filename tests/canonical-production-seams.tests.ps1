...
$reviewedAllScriptsDynamicCommandDigest='0ff25326e759f6d1b5908791470f0159cdbb8c48076e0372c9828a6b7dfe37dd'
$reviewedAllScriptsReflectionSensitiveSiteCount=14440
$reviewedAllScriptsReflectionSensitiveDigest='523a7977b54cfa76d13379eb80c17a6a7329226d28ba9af056944784e5c910fb'
...
function Invoke-ProductionSeamAnalysis {
    ...
    foreach($model in $SourceModels){
        ...
        foreach($command in @($ast.FindAll({param($node)$node -is [Management.Automation.Language.CommandAst]},$true))){
            ...
            if($commandName -ieq 'Complete-SealedHomeAuthorityBootstrap'){
                if([string]$model.RelativePath -ceq 'scripts/root-claims-registry-common.ps1' -and $null -ne $owner -and
                    [string]$owner.Name -ceq 'Complete-SealedHeldCanonicalPrivateRootBootstrap'){
                    $completionCompleteAllowedCallers.Add("$($model.RelativePath):$ownerName")
                }
                else{$fixedObservationBoundaryViolations.Add("canonical bootstrap Complete caller: $($model.RelativePath):$($ownerName):$commandName")}
            }
            if($commandName -ieq 'Complete-SealedHeldCanonicalRecoveryRootRemainder'){
                if([string]$model.RelativePath -ceq 'scripts/root-claims-registry-common.ps1' -and $null -ne $owner -and
                    [string]$owner.Name -ceq 'Complete-SealedHeldCanonicalPrivateRootBootstrap'){
                    $completionRemainderAllowedCallers.Add("$($model.RelativePath):$ownerName")
                }
            ...
            if($commandName -ieq 'Assert-AuthorityFinalIdentitiesDerivedFromIntent'){
                if([string]$model.RelativePath -ceq 'scripts/shared-authority-state-common.ps1' -and $null -ne $owner -and
                    [string]$owner.Name -ceq 'New-AuthorityStatePostimage'){
                    $finalIdentitiesAllowedCallers.Add("$($model.RelativePath):$ownerName")
                }
                else{$fixedObservationBoundaryViolations.Add("authority final identities caller: $($model.RelativePath):$($ownerName):$commandName")}
            }
            if($commandName -ieq 'Get-AuthorityStateIntentProjection'){
                if([string]$model.RelativePath -ceq 'scripts/root-claims-registry-common.ps1' -and $null -ne $owner -and
                    [string]$owner.Name -ceq 'Write-SealedRegistryCurrentEnvStatePostimage'){
                    $intentProjectionAllowedCallers.Add("$($model.RelativePath):$ownerName")
                }
    ...
    if((@($targetContextIntentAllowedCallers | Sort-Object -CaseSensitive) -join "`n") -cne ($reviewedTargetContextIntentOwnerInventory -join "`n")){
        $fixedObservationBoundaryViolations.Add('reviewed authority target-context intent owner inventory changed')
    }
    $reviewedFinalIdentitiesOwnerInventory=@('scripts/shared-authority-state-common.ps1:New-AuthorityStatePostimage')
    if((@($finalIdentitiesAllowedCallers | Sort-Object -CaseSensitive) -join "`n") -cne ($reviewedFinalIdentitiesOwnerInventory -join "`n")){
        $fixedObservationBoundaryViolations.Add('reviewed authority final identities owner inventory changed')
    }
    $reviewedIntentProjectionOwnerInventory=@('scripts/root-claims-registry-common.ps1:Write-SealedRegistryCurrentEnvStatePostimage')
    ...
    foreach($completionEntry in @(
        ...
        @('Write-SealedRegistryCurrentEnvStatePostimage','scripts/root-claims-registry-common.ps1'),
        ...
        if($completionDefinitions.Count -ne 1 -or
            [string]$completionDefinitions[0].RelativePath -cne $completionEntry[1] -or
            -not (Test-DirectScriptTopLevelFunctionDefinition -Function $completionDefinitions[0].Ast)){
            $fixedObservationBoundaryViolations.Add("$($completionEntry[0]) definition is missing, ambiguous, or outside $($completionEntry[1])")
        }
    }
