...
- [x] **Step 1: Re-run the frozen canonicalization vectors**
...
- [x] **Step 2: Check canonical-plan compatibility**
...
- [x] **Step 3: Reuse document helpers**
...
- [x] **Step 4: Record the dependency checkpoint**
...
- [x] **Step 1: Write path and link failure fixtures**
...
- [x] **Step 2: Implement stable target identity**
...
- [x] **Step 3: Implement no-follow walking**
...
- [x] **Step 4: Implement safe copy/hash/compare**
...
- [x] **Step 5: Verify all boundary cases**
...
- [x] **Step 1: Add the missing regression cases**
...
- [x] **Step 2: Split classify/transform from commit**
...
- [x] **Step 3: Derive the canonical target**
...
- [x] **Step 4: Gate the public command**
...
- [x] **Step 5: Verify normalization behavior**
...
- [x] **Step 1: Write isolation tests**
...
- [x] **Step 2: Add explicit internal preflight parameters**
...
- [x] **Step 4: Verify zero real-tree writes**
...
- [x] **Step 1: Write plan-contract failures**
...
- [x] **Step 4: Write immutable plans**
...
- [x] **Step 1: Select and validate CanonicalRecoveryRoot**

Add zero-write `agent-dotfiles.ps1 canonical status` plus `canonical setup -DryRun -PlanPath <new>` / matching `-Apply -PlanPath <existing>`, routed only to `setup-canonical-transaction.ps1` and `CanonicalOperationKind=setup`. Status reuses the no-create canonical recovery/setup locator and returns exactly `canonical-recovery-required|canonical-setup-required|canonical-ready|manual-recovery-required`; bootstrap and rollout call this same helper. Existing `scripts/setup.ps1` remains exclusively the runner-approval/inert-hook installer; it rejects canonical DryRun/Apply/PlanPath, while canonical setup rejects runner approve/install switches and never modifies hooks/approved-runner pointer. Canonical DryRun chooses current-user-only, working-tree-external private roots on the repository volume without creating them. Construct one deterministic token-SID owner/protected-DACL template; with a held no-follow directory handle bind each existing ancestor's identity/owner/DACL, the MISSING remainder and expected final template, reject broad Everyone/Authenticated Users/Builtin Users write/delete-child or inherited broad write, and require every existing final root to match the template exactly. Define `RepoId=ClaimId` deterministically from domain tag+token SID+GitCommonDir volume/directory identity so linked worktrees share it and fresh clones do not. Reuse Task 5's create-new/immutable, timestamp/nonce-free canonical setup-state/root-claim v1 serializers; setup-state's only locator is absolute resolved `git rev-parse --git-common-dir` plus literal `ai-agent-dotfiles/canonical-setup-state.json`, with linked-worktree equality, zero-or-one regular file and extra/reparse rejection. The setup plan binds exact expected global claim bytes/hash plus stable `SetupIntentHash` and `ExpectedSetupStateProjectionHash`; the projection excludes Apply-derived `Final*` identities. Phase 2 journals root creation, captures actual owner/DACL/identity, and publishes a final state containing `RootClaimHash`, the projection hash and final contexts; its full hash appears only in journal/result/COMPLETE. Status revalidates intent/claim/state/actual and rejects delete-recreate identity or owner/DACL drift. Do not mutate Phase 0 approved-runner-state v1 and do not offer replace/relocate in v1. Add linked/fresh/restart/MISSING→EXISTS/identity-drift/owner-DACL-drift/collision/extra-entry/BackupRoot-MISSING/status-zero-write/cross-command-parameter/Apply-without-plan fixtures. Reject cross-volume, root, repo-descendant, source/live/backup/control overlap, uncontrolled MISSING ancestor, or reparse ancestry. Phase 1 production canonical setup Apply returns interlocked; Phase 2 enables the same reviewed branch only after it can bootstrap ControlBase+BackupRoot, acquire global live lock, publish/finalize the matching global claim, and validate this root against every canonical/HomeAuthority claim on each use.

- [x] **Step 2: Acquire the Git-common-dir lock**
...
- [x] **Step 7: Verify locking, recovery, and hard-kill windows**
...
|---|---|
| canonical setup: exact durable claim temp after `SETUP_CLAIM_INTENT`, or exact global claim published, final setup-state MISSING | `canonical-recover-finalize` revalidates the claim's intent/projection and the journal-created roots' actual final identity/owner/DACL; a unique valid claim temp is first published to its exact create-new claim slot, then the unique matching final state is published and its full hash bound in result/COMPLETE; original setup Apply and generic rollback/forward resume are forbidden |
...
| canonical setup: pending claim without its exact durable intent, simultaneous claim+state temps, state without claim, mismatched/corrupt claim/state, changed root or unfinished ambiguity | manual-recovery-required |
| `WORKSPACE_CREATE_INTENT` with the exact container absent, or `WORKSPACE_CREATED` with its recorded empty identity | abandon before any target primitive; unjournaled, changed, non-empty, or partial workspace state is manual |
| `PREIMAGE_COPY_INTENT` with preimage absent or a complete Current-matching copy | abandon before any target primitive; a partial/mismatched copy is manual and preserved |
...
Hard-kill immediately after the canonical setup claim temp is flushed and before its create-new rename, before/after the final setup-state temp/rename, before and after each `preimage/` and `swap-old/` workspace mkdir, and before/after preimage copy including a separately retained partial-copy fixture. Hard-kill exactly before/after each parent mkdir, directory add/replace/delete rename or deletion-install record, file replace/no-overwrite move, and every inverse rollback primitive, plus immediately before/after `POSTCONDITIONS_OK`, fixed result and final `Phase=COMPLETE` record publish. Prove the transaction-wide preparation barrier rejects the first target primitive until every non-parent target has a durable Current-matching preimage/MISSING record; descending rollback treats a later PRE_PRIMITIVE target as a verified byte-preserving no-op. Also retain an isolated UNPREPARED no-op recovery fixture. In linked worktree A create/kill a transaction, then prove worktree B's status finds it, new mutation is blocked, the TransactionId resolves uniquely, reviewed recovery succeeds from B, and original/closing plan replay is rejected globally. Race an external manifest edit between final precheck and atomic replace; require either plan-stale before primitive or manual recovery with the raced bytes captured in swap-old, never silent loss. Exercise status, the one allowed reviewed action, stale recovery plans, every terminal Outcome, and injected recovery failure at every row; never offer forward resume.
...
- Modify: `tests/canonical-transaction.tests.ps1`
- Create: `tests/canonical-transaction-apply.tests.ps1`

- [x] **Step 1: Stage verified target bytes**
...
- [x] **Step 2: Execute all targets through the journal**
...
- [x] **Step 3: Verify postconditions without writing real output**
...
- [x] **Step 4: Finalize or retain recovery**
...
**Artifacts / Locations:**
- Create: `scripts/canonical-skill-adapter-common.ps1`
- Modify: `scripts/normalize-skill.ps1`
...
- [x] **Step 1: Route every canonical write through one CLI**
...
- [x] **Step 2: Remove legacy mutation paths**
...
- [x] **Step 3: Preserve current classification/inventory behavior**
...
- [x] **Step 4: Verify batch atomicity**
