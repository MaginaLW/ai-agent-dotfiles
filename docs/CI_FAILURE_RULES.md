...
- **平台另有硬上限，2026-09-19 起由 shard 结构消解**：GitHub 文档规定「Each job in a workflow
  ...
  27555 秒，2026-09-26 按 R2 重算为 504.25 分钟 / 30255 秒）；所有者已裁决**拆分测试矩阵**：
...
- ~~**聚合预算已越过平台作业上限**（R2）~~ **已收口（2026-09-19，所有者裁决拆分测试矩阵）**：
  ...
  42 套件的已证预算 30255 秒（504.25 分钟；2026-09-19 拆分时为 27555 秒）被拆进三个并行作业，
  每个作业的合同秒数（shard 预算 + 300 + 120）分别为 8700/10200/12195 秒
  （145/170/203.25 分钟），对应声明 `timeout-minutes` 170/185/230，全部低于 360 分钟平台上限。
  现行规则见 R2。
