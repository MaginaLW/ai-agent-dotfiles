...
（623.60 秒，门槛 85% 达成）；`diff-cover --compare-branch HEAD^ --fail-under=90` 退出 0（纯
文档改动，diff 内无覆盖率行）；`ruff check`、`ruff format --check`（436 文件）、`mypy`（41
源文件）、`git diff --check` 全部退出 0。写后复核：HEAD 即本窗口提交，`77989c2` 仍在祖先链上，
工作区只含并行会话既有的未跟踪方案文件。
