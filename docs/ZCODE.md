...
### 第十三次闭环执行（2026-09-17，独立评审与规则修正窗口）

窗口任务：按所有者指示让 Grok 独立复核本窗口产出。三路 `grok-4.6` 并行（各自一次性 detached
worktree @ `92d3561`、完全访问、报告留在检出外）：事实重算、预算改动复核、规则文件对抗性评审。
事实面全部复核一致（123 次运行 / 57 次失败、失败步骤分布、55+2 annotation、22 次超时与 10 个套件
的分布、doctor 五次同一 `[FAIL] status\active`、9+1 schema 报文、两次 gitleaks 及其提交、
`5807727a`→`71b8e74` 的 `tests/` 空 diff）；唯一无法独立核验的是 `881047a` 的本地实测四元组
231/187/1511/197 秒（来源循环），规则文件已改标为「本仓自述」。据此修正规则文件（R2 的倍率叙事与
条件式工作流条款、R3 改为同一 SHA 判据且明确不得读成「可忽略」、R8 去掉可被误读为规避扫描器的
写法、R1–R10 判据收紧、处置流程改为按失败步骤名分流、步骤数更正为 13），在
`status/active/live-safety-hardening.md` 记录评审结论并追加待办 12，`CLAUDE.md` 补上同一指针。

新反馈（已提交上游 `d40971a`，两条，合入上游 `docs/operations/adoption.md` 收尾方法第 4、6 项）：

1. **区分「配置声明的上限」与「平台实际执行的上限」。** 声明值可以高于平台硬上限，那样合同断言
   对它成立并不代表运行能走完；越界记为决策项，不直接改结构。实例：工作流声明
   `timeout-minutes: 460`，托管 runner 作业上限 6 小时，重算后的已证预算 439.25 分钟已越过。
2. **评审结论本身也要核验。** 独立模型或 worker 的断言不因来源独立而免检；不成立的条目要明确
   记为「不采纳」并写出理由。

上游验证（在本仓同级、身份已核对的检出内，候选提交 `d40971a` 的已提交字节）：`uv run --locked
ruff check`、`ruff format --check`（436 文件）、`mypy`（41 源文件）退出 0；`uv run --locked python
-m pytest -q --cov=aiflow --cov-branch --cov-fail-under=85` **1945 通过、总覆盖率 88.12%**
（本窗口实跑 756.46 秒）；`diff-cover --compare-branch 7f0f4a7 --fail-under=90` 退出 0（纯文档改动，
diff 内无覆盖率行）；`git diff --check` 退出 0。写后复核：上游 HEAD 即本窗口提交 `d40971a`，
`7f0f4a7` 仍在直接祖先链上，工作区仍只含并行会话既有的三个未跟踪方案文件。

版本回灌核对：上游 HEAD 由上次应用的 `7f0f4a7` 直接推进到本窗口的 `d40971a`，链路连续、无分叉、
无版本漂移；除本窗口新增文字外，方法文件无其他适用差异，本页不复制其正文。
